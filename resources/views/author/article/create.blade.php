...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div id="loadingScreen" style="display:none">
            ...
            <div class="spinner-border text-primary flex items-center justify-center" role="status"
        ...
        <!-- loading screen end -->
        <form id="needs-validation" class="needs-validation" novalidate action="{{ URL::route('author.' . $url . '.store') }}" method="post"
            ...
            <div class="row">
                <div class="col-xl-8 col-lg-8" style="display: none;" id='terms'>
                    <div class="form-group">
                        ...
                        <ul>
                            <li>Submission Requirements You must read and acknowledge that you've completed the requirements
                                below before proceeding</li><br>
                            <li>The submission has not been previously published, nor is it before another journal for
                                consideration (or an explanation has been provided in Comments to the Editor).</li><br>
                            <li>The submission file is in Microsoft Word document file format. (PDF not accepted).</li><br>
                            <li>All references were prepared as per the journal guidelines. Where available, DOI or URLs for
                                the references have been provided and cross cited in manuscript body text. The text adheres
                                to the stylistic and bibliographic requirements outlined in the Author Guidelines of the
                                respective journal.</li><br>
                            <li>I/We agreed to pay Article Processing Charges (APC) if paper accepted. Also agreed that the
                                total number of pages will not exceed more than 15 pages in our paper. (If exceeded
                                additional charges shall be applied). India: APC as per the journal- with DOI, Without DOI
                                excluding 18% GST; | Other than India: $50; Extra Pages-$3 per page. No refund is possible
                                for any reasons if the article is published online.</li><br>
                            <li>Any changes are not possible after the publication of your paper. Please send the paper in
                                Journal template (or) Journal format to avoid any mistakes. Please make sure similarities
                                should be less than 30%. All the copyright of your article will be transferred & reserved to
                                publish in a respective journal.</li><br>
                            <li>Journal is responsible only for publication of papers on the Journal website which is
                                mentioned on acceptance letter. Since, indexing of the journals is unstable, it is the
                                responsibility of the author to check their requirements on a timely basis.</li>
                        </ul>

                    </div>
                </div>
                <div class="col-xl-3 col-lg-3" style="display: none;" id='termsimage'>
                    <div id="show_image" class="d-flex align-content-center justify-content-center mx-auto">
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-8 col-xs-12" style="margin-bottom: 20px;">
                    <div class="form-group">
                        ...
                        <input type="file" class="form-control" name="file" id="file" placeholder="Upload Document"
                            required>
                        <p></P>
                        <p style="color:red"> (Only PDF or Word Document format allowed; File size should be less than 5MB)
                        </p>
                        <div class="invalid-feedback">
            ...
            $('#file').on('change', function () {
                ...
                if (file) {
                    ...
                    if (validExtensions.includes(fileType)) {
                        ...
                        // If the file type and size are OK, you can proceed (e.g., with further processing or upload)
                    } else {
    ...
    <script>
$(document).ready(function() {
    const formSelector = "#needs-validation"; // Update with your form's actual ID or class
    const formKey = "autosave_form_data";

    // Restore saved form data on load
    if (localStorage.getItem(formKey)) {
        let savedData = JSON.parse(localStorage.getItem(formKey));
        $.each(savedData, function(name, value) {
            let $field = $(`${formSelector} [name="${name}"]`);
    ...
    $(`${formSelector} :input`).on("input change", function() {
        let formData = {};
